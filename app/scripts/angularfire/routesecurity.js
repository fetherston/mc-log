(function(angular) {
   ...
   function RouteSecurityManager($location, $rootScope, $route, path) {
      ...
      this._admin = !!($rootScope.auth && $rootScope.auth.user.admin);
      this._init();
   }
   ...
   RouteSecurityManager.prototype = {
      _init: function() {
         ...
         self._rootScope.$on("$routeChangeStart", function(e, next) {
            ...
            self._adminRequiredRedirect(next, self._loginPath);
         });
      ...
      _checkCurrent: function() {
         ...
         if (this._route.current) {
            ...
            this._adminRequiredRedirect(this._route.current, this._loginPath);
         }
      },
      ...
      // A function to check whether the current path requires admin rights,
      // and if so, whether a redirect to home.
      _adminRequiredRedirect: function(route, path) {
         if (route.adminRequired && !this._admin){
            if (route.pathTo === undefined) {
               this._redirectTo = this._location.path();
            } else {
               this._redirectTo = route.pathTo === path ? "/" : route.pathTo;
         ...
         else if( this._admin && this._location.path() === this._loginPath ) {
            this._redirect('/');
         }
      }
   };
